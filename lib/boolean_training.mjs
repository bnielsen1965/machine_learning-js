...
export class BooleanTraining {
  ...
  */
  static async train (neuron, dataSet, rate, tweak, runs, target, progress) {
    let error;
    let count = 0;
    while (++count < runs) {
      ...
      error = adjustError;
      if (progress && typeof progress == "function" && !(count % 1000)) {
        progress(neuron, count, error);
        await BooleanTraining.delay(10);
      }
    }
    return { error, count };
  }
  ...
  * @param {Number} target The target error value for the model.
  */
  static async trainNetwork (network, dataSet, rate, tweak, runs, target, progress) {
    let error;
    let count = 0;
    while (++count < runs) {
      // get current model error
